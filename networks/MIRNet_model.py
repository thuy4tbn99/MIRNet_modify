...
class SKFF(nn.Module):
    ...
    def forward(self, inp_feats):
        # print('inp_feats', type(inp_feats), inp_feats[0].shape)
        ...
        feats_U = torch.sum(inp_feats, dim=1)   # element-wise sum 
        feats_S = self.avg_pool(feats_U)        # GAP
...
class UpSample(nn.Module):
    def __init__(self, in_channels, scale_factor, stride=2, kernel_size=3):
        ...
        modules_body = []
        for i in range(self.scale_factor): # 4x sampling -> apply log2(4)=2 lần
...
class MSRB(nn.Module):
    ...
    def forward(self, x):
        inp = x.clone()
        #col 1 only
        blocks_out = [] # after pass through DAU
        for j in range(self.height):
            if j==0:
                inp = self.blocks[j][0](inp)
            else:
                # print("self.blocks[j][0]", self.blocks[j][0])
                # print("inp:", inp.shape)
                # print("self.down[f'{inp.size(1)}_{2}'](inp)", self.down[f'{inp.size(1)}_{2}'](inp).shape)
...
class MIRNet(nn.Module):
    ...
    def forward(self, x):
        # print('x', x.shape) # (2, 3, 128, 128) * (4 core) -> (8, 3, 128, 128)
        h = self.conv_in(x)
        # print('after conv_in: x', h.shape)      # (2, 32, 128, 128)
        h = self.body(h)
        # print('after body: ', h.shape)          # (2, 32, 128, 128)
        h = self.conv_out(h)
        # print('after conv h: ', h.shape)        # (2, 3, 128, 128)
        h += x
        return h
